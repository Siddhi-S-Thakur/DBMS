DROP DATABASE IF EXISTS college_aggregation_db;

CREATE DATABASE college_aggregation_db;
USE college_aggregation_db;
...
-- Q2
ALTER TABLE Enrollment ADD COLUMN Remarks VARCHAR(50);
-- Q3
SELECT Course_id FROM Enrollment GROUP BY Course_id HAVING AVG(Attendance) > 80;
-- Q4
SELECT MAX(CGPA) AS maximum_cgpa, MIN(CGPA) AS minimum_cgpa FROM Student;
-- Q5
DROP VIEW IF EXISTS enrollment_summary_view;
CREATE VIEW enrollment_summary_view AS SELECT Enroll_id, Student_id, Course_id FROM Enrollment;
